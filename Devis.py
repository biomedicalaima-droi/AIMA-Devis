...
class AIMA_PDF(FPDF):
    ...
    def draw_first_page_info(self, doc_num, ref_text, selected_date, client_name, client_address, aima_info, status, realized_by, pay_mode):
        ...
        y_boxes = 38; self.set_xy(10, y_boxes); self.set_fill_color(51, 139, 140); self.set_text_color(255, 255, 255); self.set_font('Arial', 'B', 9)
        self.cell(80, 7, "Association AIMA", 1, 1, 'C', True); self.set_text_color(0, 0, 0); self.set_font('Arial', '', 8); self.set_x(10)
        self.multi_cell(80, 4, aima_info.encode('latin-1', 'replace').decode('latin-1'), 1, 'C')
        y_left_end = self.get_y(); self.set_xy(120, y_boxes); self.set_fill_color(51, 139, 140); self.set_text_color(255, 255, 255)
        self.cell(80, 7, f"DESTINATAIRE : {client_name.upper()}", 1, 1, 'C', True); self.set_text_color(0, 0, 0); self.set_font('Arial', '', 9); self.set_x(120)
        self.multi_cell(80, 5, client_address.encode('latin-1', 'replace').decode('latin-1'), 1, 'C')
        y_right_end = self.get_y(); y_meta = max(y_left_end, y_right_end) + 4
        self.set_xy(10, y_meta); self.set_font('Arial', '', 8.5)
...
if st.sidebar.button("🔄 Réinitialiser tout"):
    st.session_state.manual_items_dict = []; st.session_state.active_catalog = []; st.session_state.catalog_selector = []; st.session_state.counters = {"DEVIS": {}, "FACTURE": {}}; st.rerun()

logo_base64 = get_base64_logo(AIMA_LOGO_PATH)
with st.container():
    col_l, col_c, col_r = st.columns([1, 3, 1])
    with col_c:
        logo_html = f'<div style="text-align: center;"><img src="{logo_base64}" width="180"></div>' if logo_base64 else ""
        st.markdown(f'{logo_html}<h1 style="text-align: center; color: #184973;">AIMA - Gestion de Devis & Factures</h1>', unsafe_allow_html=True)
...
c_name = st.sidebar.text_input("Client", value="")
c_addr = st.sidebar.text_area("Adresse Client", value="")

# LOGIQUE DE NUMÉROTATION SÉPARÉE
prefix = "FAC" if doc_type == "FACTURE" else "DEV"
client_clean = c_name.strip().upper().replace(" ", "-") if c_name else "CLIENT"

if client_clean not in st.session_state.counters[doc_type]:
    st.session_state.counters[doc_type][client_clean] = 1
...
include_liv = st.sidebar.checkbox("Livraison par nos soins au pied de l'immeuble ", value=True)
...
with col_cat: selected_catalog = st.multiselect("📦 Sélectionner les dispositifs :", options=sorted(list(data_prices.keys())), key="catalog_selector")
with col_imp:
    uploaded_pdf = st.file_uploader("📥 Importer depuis PDF", type="pdf")
    if uploaded_pdf and st.button("🚀 Extraire Articles"):
...
with cm1: custom_name = st.text_input("Désignation", placeholder="Nom de l'article...")
with cm2: custom_price = st.number_input("Prix P.U.", min_value=0.0, format="%.2f")
with cm3: 
    st.write(" "); 
    if st.button("✅ Ajouter"):
        if custom_name: st.session_state.manual_items_dict.append({"id": f"custom_{time.time()}", "nom": custom_name, "prix": custom_price}); st.rerun()
...
st.sidebar.markdown("---"); st.sidebar.markdown(f"### **TOTAL NET : {grand_total:,.2f} EUR**")
...
if items_to_pdf and st.button(f"📄 GÉNÉRER {doc_type} PDF"):
    ...
    cols_w = [60, 20, 15, 25, 50, 20] if doc_type == "DEVIS" else [125, 20, 15, 30]
    headers = ["Designation", "P.U.", "Qte", "Total", "Photos", "Lieu"] if doc_type == "DEVIS" else ["Designation", "P.U.", "Qte", "Total"]

    # En-têtes (gris léger, sans gras pour uniformiser)
    pdf.set_font('Arial', '', 9); pdf.set_fill_color(240, 240, 240); pdf.set_xy(10, y_pos)
    for i, h in enumerate(headers): pdf.cell(cols_w[i], 8, h, 1, 0, 'C', True)
    pdf.ln()

    # Corps du tableau (Standardisation du format)
    ...
    for row in items_to_pdf:
        ...
        h_row = max(nb_lines * 4 + 2, 32 if (doc_type == "DEVIS" and row['Images']) else 8)
        
        if pdf.get_y() + h_row > 260: 
            pdf.add_page(); pdf.set_font('Arial', '', 9); pdf.set_fill_color(240, 240, 240)
            for i, h in enumerate(headers): pdf.cell(cols_w[i], 8, h, 1, 0, 'C', True)
            pdf.ln(); pdf.set_font("Arial", '', 9)

        y_c = pdf.get_y(); pdf.rect(10, y_c, cols_w[0], h_row)
        pdf.set_xy(10, y_c + (h_row - (nb_lines * 4)) / 2); pdf.multi_cell(cols_w[0], 4, nom_p, 0, 'L')
        pdf.set_xy(10 + cols_w[0], y_c); pdf.cell(cols_w[1], h_row, f"{row['P.U.']:,.2f}", 1, 0, 'C') 
        pdf.cell(cols_w[2], h_row, str(row['Qté']), 1, 0, 'C')
        pdf.cell(cols_w[3], h_row, f"{row['Total']:,.2f}", 1, 0, 'C') 
        
        if doc_type == "DEVIS":
            img_x = pdf.get_x(); pdf.cell(cols_w[4], h_row, "", 1, 0)
            if row['Images']:
                for idx, img_file in enumerate(row['Images']):
                    ...
                    try:
                        ...
                        pdf.image(tmp.name, img_x + 2 + (idx * 16), y_c + 2, w=14, h=h_row - 4)
                    finally: tmp.close(); os.remove(tmp.name)
            pdf.cell(cols_w[5], h_row, str(row['Lieu']).encode('latin-1', 'replace').decode('latin-1'), 1, 1, 'C')
        else: pdf.ln(h_row)

    pdf.ln(3); y_final = pdf.get_y(); summary_x = 10 if doc_type == "DEVIS" else 110
    
    # Récapitulatif uniformisé
    pdf.set_font("Arial", '', 8.85)
    summary_data = []
    if include_adh: summary_data.append((f"Cout adhesion annuelle {d_date.year}", "1.00 EUR"))
    if include_liv: summary_data.append(("Livraison par nos soins au pied de l'immeuble", f"{liv_total:,.2f} EUR"))
    if include_remise_globale and montant_remise_globale > 0: summary_data.append(("Remise", f"- {montant_remise_globale:,.2f} EUR"))
    
    for label, val in summary_data:
        pdf.set_xy(summary_x, pdf.get_y()); pdf.cell(65, 6, label.encode('latin-1','replace').decode('latin-1'), 1, 0, 'L'); pdf.cell(25, 6, val, 1, 1, 'C')
    
    pdf.set_xy(summary_x, pdf.get_y()); pdf.set_fill_color(51, 139, 140); pdf.set_text_color(255, 255, 255); pdf.set_font("Arial", 'B', 10)
    pdf.cell(65, 10, "TOTAL NET", 1, 0, 'C', True); pdf.set_text_color(0, 0, 0); pdf.cell(25, 10, f"{grand_total:,.2f} EUR", 1, 1, 'C')

    if doc_type == "DEVIS":
        pdf.set_xy(120, y_final); pdf.set_font("Arial", '', 9); pdf.cell(80, 8, "Signature :", 1, 1, 'L'); pdf.set_x(120); pdf.cell(80, 20, "", 1, 1)
